...
def prose_words(text: str, kind: str) -> list[str]:
    if kind == "draft":
        ...
        text = text[text.index(r"\begin{abstract}") : text.index(r"\bibliographystyle")]
        ...
        text = re.sub(
            r"\\(?:section\*?|subsection|paragraph|caption)(?:\[[^]]*\])?\{([^{}]*)\}",
            r" \1 ",
            text,
        )
...
def main() -> int:
    ...
    for text, label in [(main_tex, "LaTeX"), (draft_md, "Markdown")]:
        ...
        audit.check(
            f"nine-arm convention in {label}",
    ...
    # Bidirectional semantic check. Citations, math syntax, headings, and table
    # layout are stripped; four-word prose n-grams remain deliberately strict.
