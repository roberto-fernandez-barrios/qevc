...
print("\nDescriptive Spearman rho across structured environments (n=%d):" % len(rows))
print("  IID p-values are omitted: environments share nuisance-family structure/CRN.")
for gcol in ("q_mmd2", "q_erank_ratio", "r_mmd2"):
    for dcol in ("d_qk", "d_xgb", "d_rbf"):
        ...
        rho, _ = stats.spearmanr(v, u)
        print(f"  {gcol:14s} vs |{dcol}|: rho={rho:+.3f}")
