...
because the raw matrices are indefinite. Entry-wise finite-shot estimation
therefore does not necessarily produce a PSD kernel or a standard RKHS Gram.
...
raw matrix, but the usual convex PSD-kernel interpretation and guarantees do
not automatically attach to that solution; indefiniteness alone also does not
make its numerical deployment meaningless.
...
full-rank training block also gives a coherent linear extension of every
unchanged source/target cross column through the training span. Each repaired
deployment is then refitted, Platt-recalibrated, threshold-refrozen and audited
on exactly the same roles, claim grids and paired audit streams. No Gram,
sample, seed or QPU job is newly drawn.
...
zero far-margin deployment-relative rate, ideal-anchored heterogeneity,
