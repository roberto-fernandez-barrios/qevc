...
Any test of size at most $\alpha$ based on those observations has power equal
to its actual size and hence at most $\alpha$ against the nuisance (exactly
$\alpha$ only for exact size $\alpha$). No nontrivial distinction is possible;
under our fail-closed policy, absence of distinguishing evidence is reported
UNRESOLVED for claims whose truth changes only through hidden rate weights.
...
failure of a sufficient condition gives no conclusion. On coverage, any
decisive verdict points to that sign. Equality of two realized ternary
verdicts additionally requires that both audits resolve and that both CSs
cover: without joint calibration the intersection probability is at least
$1-2\alpha$, or $1-\alpha$ if each audit uses $\alpha/2$. For weighted claims the
...
E16 archives only $\Delta M_S$, not $\Delta M_T$ or
$\Delta M_T-\Delta M_S$; Figure~\ref{fig:s16} and Table~\ref{tab:e16}
therefore do not instantiate either condition. The proposition remains
conditional, while archived flip rates are independent empirical evidence.
...
movement $|\Delta M_S|$. The target movement $\Delta M_T$ and the difference
$\Delta M_T-\Delta M_S$ were not archived, so this panel cannot instantiate
Proposition 4. Source:
...
\centering
\footnotesize
\caption{E16 shot-budget summary. Frobenius error and effective rank are means over five deployments; movement is only the maximum absolute weighted source-reference shift $|\Delta M_S|$, not either quantity needed by Proposition 4. Flip and abstention entries are independent empirical percentages. Source: \texttt{results/tables/E16\_quantum\_uncertainty.json}.}
\label{tab:e16}
