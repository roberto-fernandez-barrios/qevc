...
\footnotesize
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.09\linewidth}>{\raggedright\arraybackslash}p{0.19\linewidth}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.26\linewidth}>{\raggedright\arraybackslash}p{0.14\linewidth}@{}}
\caption{All nine registered falsifier arms that fired. Frozen conditions and
consequences are recorded in \texttt{docs/experiment\_registry.md}. The
freeze/provenance column gives the short Git commit and date at which the
frozen condition entered the public repository; every listed commit precedes
the corresponding archived run manifest. E02R's registration and launch share
one commit (the archived run began 25 minutes later), so it is documented at
launch rather than strictly predeclared.}\label{tab:falsifiers}\\
\toprule
Arm & Frozen condition & Observed firing & Consequence obeyed & Freeze / provenance \\
\midrule
\endfirsthead
\toprule
Arm & Frozen condition & Observed firing & Consequence obeyed & Freeze / provenance \\
\midrule
\endhead
E02R & TES sign pattern fails multi-seed replication & Up-shift monotonicity in only 2/5 seeds & Down-shift retained provisionally within-world; E17 later withdrew the directional cross-world claim. & \texttt{ce6734d}, 2026-08-10 (at launch) \\
E12(e) & Two flagship decoupling cells fail to reproduce & Diboson flagship cells stayed nominal-like in the fresh world & Normalization-collapse statement scoped to signal-region composition; failing registered cells kept. & \texttt{f6d99cb}, 2026-08-11 \\
E14 v1 & Control-region interval coverage below nominal & Pure-Poisson fit gave ttbar bias $+0.07$ and zero coverage & Run blocked; template-statistics likelihood registered, then rerun as v2. & \texttt{f6d99cb}, 2026-08-11 \\
E15 gate & Nominal L2 coverage outside $0.6827\pm0.02$ & Conditional-ensemble and numerical-conditioning implementations each failed & Both grids blocked; ensemble semantics and optimizer fixed before any shifted claim. & \texttt{f6d99cb}, 2026-08-11 \\
E17(b) & Degradation signs fail across additional worlds & One world improved QK under combo by 0.009; another under TES-down by 0.005 & Cross-world degradation claim withdrawn. & \texttt{1d6d93d}, 2026-08-11 \\
E08v2(a) & Corrected counting coverage outside the nominal band & One-draw corrected coverage $0.9916$--$1.000$ & Arm blocked and bounded multi-draw E08v3 registered. & \texttt{1d6d93d}, 2026-08-11 \\
E08v2(b) & Independent-template flagship L2 coverage $<0.633$ & Coverage 0.000 versus shared 0.7188 & ``Profiling restores validity'' made explicitly shared-simulation-conditional. & \texttt{1d6d93d}, 2026-08-11 \\
E08v3(a) & Marginal symmetric-BB coverage outside $0.6827\pm0.06$ for at least two gated models & All three gated models overcovered, 0.7513--0.8284 & Calibration-restoration claim withdrawn; conservative direction reported. & \texttt{1faecfc}, 2026-08-12 \\
E13v2(b) & All true benchmark BA claims unresolved by 5,000 labels & TPR component 0/200 resolutions in every cell & BA path published as information-limited; TNR success and mechanism reported separately. & \texttt{1d6d93d} + \texttt{309025f}, 2026-08-11/12 \\
\bottomrule
\end{longtable}
\normalsize
\FloatBarrier
...
\small
\begin{longtable}{>{\raggedright\arraybackslash}p{0.19\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}p{0.51\linewidth}}
\caption{Audit-trail summary. The full findings and dispositions are in \texttt{docs/audits/}.}\label{tab:audit-trail}\\
\toprule
...
Editorial focus / concision patch, 2026-09-01 & Main-text structure and wording, Supplement Table~S1 provenance, release metadata & Proposition-3 statement freed of the general stability-ordering clause; abstract scopes the classical controls to nominal-performance and sensing effects; the finite-template condition stated with every Contribution-2 coverage claim; internal experiment identifiers replaced by descriptive language in the main text; acronyms defined; main text shortened; freeze/provenance column added to Table~S1. No number, table value, figure file or scientific artifact changed. \\
\bottomrule
\end{longtable}
\normalsize
\FloatBarrier
